...
type NetConfig struct {
	BindAdress string `json: "bind_address"` // tag 지정 (json 파일에서 어떤 형식을 읽어올 것인가)
	Port       int    `json: "port"`
}
...
type TcpSession struct { // TcpSessionManager에 의해 관리됨.
	SeqIndex       uint64   // sessionuUniqueId
	Index          int32    // sessionId
	Conn           net.Conn // file descriptor
...
type TcpSessionManager struct {
	NetworkFunctor   SessionNetworkFunctor
	SessionMap       sync.Map
	SessionIndexPool *Deque // 여기서 값을 받아옮.
}

func StartServiceBlock(netConfig NetConfig, networkFunctor SessionNetworkFunctor) {
	fmt.Println("TCP Server Start on 8000 port")
	/* TcpSession 매니저 생성 */
	_tcpSessionManager = createSessionManager(networkFunctor)
	// createSessionManager() [PATH]: [./tcpSession.go]
	// session들을 관리할 매니저

	address := fmt.Sprintf(":%d", netConfig.Port)
	l, err := net.Listen("tcp", address)
	// net.Listen은 IP를 알아서 지정해주는 건가?
	// 서버니까 어차피 현재 실행 중인 localhost로 잡으면 되니까..
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	for {
		...
		for {
			...
			*/
			...
			/*
				링버퍼 구조를 이루고 있음
				startRecvPos는 패킷을 만들기 위한 길이까지만 설정하고
...
/*
만약 scale out시에 redis로 유니크값 관리하면 좋을 듯
*/
